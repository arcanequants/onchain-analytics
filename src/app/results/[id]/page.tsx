...
function CategoryCard({ category }: { category: CategoryScore }) {
  ...
  return (
    <div
      className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-lg p-4 transition-all hover:border-[var(--accent-primary)] hover:shadow-md cursor-pointer"
    ...
    >
      ...
      <div className="mt-2 flex justify-between items-center text-xs text-[var(--text-tertiary)]">
        <span>Weight: {Math.round(category.weight * 100)}%</span>
        <span>Contribution: {category.contribution.toFixed(1)} pts</span>
      </div>
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-[var(--border-primary)]">
          {category.insights.length > 0 && (
            <div className="mb-3">
              <h5 className="text-xs font-medium text-[var(--text-secondary)] mb-2">Insights</h5>
          ...
          {category.suggestions.length > 0 && (
            <div>
              <h5 className="text-xs font-medium text-[var(--text-secondary)] mb-2">Suggestions</h5>
              <ul className="space-y-1">
...
function RecommendationCard({ recommendation, index }: { recommendation: Recommendation; index: number }) {
  ...
  return (
    <div
      className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-lg p-4 transition-all hover:border-[var(--accent-primary)] hover:shadow-md"
    >
      <div className="flex items-start gap-4">
        <div className="w-8 h-8 rounded-full bg-[var(--accent-primary)]/20 flex items-center justify-center text-[var(--accent-primary)] font-bold text-sm shrink-0">
          {index + 1}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-[var(--text-secondary)] mb-3">{recommendation.description}</p>
          ...
          {isExpanded && (
            <div className="mt-4 pt-4 border-t border-[var(--border-primary)]">
              <p className="text-xs text-[var(--text-tertiary)] mb-3 italic">{recommendation.rationale}</p>

              <h5 className="text-xs font-medium text-[var(--text-secondary)] mb-2">Action Items</h5>
              ...
              {recommendation.successMetrics && recommendation.successMetrics.length > 0 && (
                <div className="mt-4">
                  <h5 className="text-xs font-medium text-[var(--text-secondary)] mb-2">Success Metrics</h5>
                  <ul className="space-y-1">
                    {recommendation.successMetrics.map((metric, i) => (
                      <li key={i} className="text-sm text-[var(--text-tertiary)]">
                        - {metric}
                      </li>
                    ))}
                  </ul>
...
function BenchmarkChart({ benchmark, currentScore }: { benchmark: ScoreResult['benchmark']; currentScore: number }) {
  ...
  return (
    <div className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-lg p-6">
      ...
      <div className="relative h-8 bg-[var(--bg-tertiary)] rounded-full overflow-hidden mb-4">
        {/* Range indicator */}
        <div className="absolute inset-y-0 left-0 right-0 flex items-center">
          {/* Bottom performer */}
          <div className="absolute left-0 w-2 h-full bg-red-500/50" />
          ...
          {/* Top performer */}
          <div className="absolute right-0 w-2 h-full bg-green-500/50" />

          {/* Current score marker */}
          <div
            className="absolute w-4 h-4 rounded-full bg-[var(--accent-primary)] shadow-lg transform -translate-x-1/2 transition-all"
      ...
      <div className="grid grid-cols-2 gap-4 text-center">
        <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
          <div className="text-2xl font-bold text-[var(--accent-primary)]">{benchmark.percentileRank}th</div>
          <div className="text-xs text-[var(--text-tertiary)]">Percentile</div>
        </div>
        <div className="p-4 bg-[var(--bg-tertiary)] rounded-lg">
          <div className="text-2xl font-bold text-[var(--text-primary)]">{benchmark.positionLabel}</div>
          <div className="text-xs text-[var(--text-tertiary)]">Position</div>
        </div>
      </div>
    </div>
  );
          ...
          <div key={provider.provider}>
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center gap-3 text-xs text-[var(--text-tertiary)]">
                <span>{Math.round(provider.mentionRate * 100)}% mentions</span>
                {provider.averagePosition && (
                  <span>Avg pos: {provider.averagePosition.toFixed(1)}</span>
                )}
              </div>
            </div>
            <ScoreBar score={provider.score} showValue animate />
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="space-y-3">
        {sortedIntents.map((intent) => (
          <div key={intent.intent} className="flex items-center gap-3">
            <div className="w-28 text-sm text-[var(--text-secondary)]">
    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <header className="bg-[var(--bg-secondary)] border-b border-[var(--border-primary)] sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button className="px-4 py-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
              Export PDF
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <section className="mb-8">
          <div className="bg-gradient-to-br from-[var(--bg-card)] to-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-2xl p-8">
            <div className="flex flex-col lg:flex-row items-center gap-8">
              ...
              <div className="flex-1 text-center lg:text-left">
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4">
                  <div className="px-4 py-2 bg-[var(--bg-tertiary)] rounded-lg">
                  ...
                  <div className="px-4 py-2 bg-[var(--bg-tertiary)] rounded-lg">
                    <span className="text-xs text-[var(--text-tertiary)]">Industry Rank</span>
                    <div className="text-lg font-semibold text-[var(--text-primary)]">
                      {scoreResult.benchmark?.percentileRank}th %ile
                    </div>
                  </div>
                  <div className="px-4 py-2 bg-[var(--bg-tertiary)] rounded-lg">
                    <span className="text-xs text-[var(--text-tertiary)]">Projected</span>
        ...
        {/* Tab Navigation */}
        <nav className="mb-6">
          <div className="flex gap-1 p-1 bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-primary)] inline-flex">
            {[
                ...
                <ul className="space-y-3">
                  {scoreResult.keyInsights.map((insight, i) => (
                    <li key={i} className="flex items-start gap-3 text-[var(--text-secondary)]">
                      <span className="mt-1 w-1.5 h-1.5 rounded-full bg-[var(--success)] shrink-0" />
                      {insight}
                    </li>
                  ))}
                </ul>
        ...
        {activeTab === 'categories' && (
          <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {scoreResult.categories.map((category) => (
                <div key={category.category} className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-lg p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-lg font-semibold text-[var(--text-primary)]">{category.name}</h3>
                      <p className="text-xs text-[var(--text-tertiary)]">Weight: {Math.round(category.weight * 100)}%</p>
                    </div>
        ...
        {activeTab === 'recommendations' && (
          <div className="space-y-6">
            {/* Summary */}
            <div className="bg-gradient-to-r from-[var(--accent-primary)]/10 to-[var(--accent-primary)]/5 border border-[var(--accent-primary)]/30 rounded-lg p-6">
              <div className="flex items-center gap-4 mb-4">
                <div className="text-4xl font-bold text-[var(--accent-primary)]">{recommendations.length}</div>
                <div>
        ...
        {activeTab === 'details' && (
          <div className="space-y-8">
            ...
            {/* Technical Details */}
            <div className="bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-lg p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Technical Details</h3>
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex justify-between py-2 border-b border-[var(--border-primary)]">
                  ...
                  <span className="text-[var(--text-primary)]">{scoreResult.benchmark?.industryName}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="mt-16 py-8 border-t border-[var(--border-primary)] bg-[var(--bg-secondary)]">
        <div className="max-w-7xl mx-auto px-4 text-center text-sm text-[var(--text-tertiary)]">
